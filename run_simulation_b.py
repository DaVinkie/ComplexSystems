...
import sys, os
from cromosim.cromosim import *
from cromosim.cromosim.micro import *
...
settings_file = "Maps_with_obstacles/Corridor_bidirectional/settings_b.json"
settings = pandas.read_csv('settings_b.csv')
with open("simulation_b_loop.txt") as f:
    loop = int(f.read())
...
while loop < len(settings):
    ...
    with open("simulation_b_loop.txt", "w") as f:
        f.write(str(loop))
    print(loop)
    
    # data settings
    output_dir    = "results/" + "b/" + str(settings["condition"][loop])
    map_condition = "Maps_with_obstacles/Corridor_bidirectional/b" + str(settings["condition"][loop]) + ".png"
    ...
    ### ADDED BY US: ######################################
        # Temporal addition:
        if(people_at_spawn_b(all_people[name]) < 8):
            if adding:
                seed += 1
