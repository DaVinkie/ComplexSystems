output_dir <- "results"
max_time   <- 99
settings   <- read.csv("soc_dynamics/settings_a.csv")
...
throuput_means <- lapply(conditions_ma[c("a/0", "a/1", "a/2", "a/3", "a/4", "a/5")], apply, 1, mean)
throuput_ses   <- lapply(conditions_ma[c("a/0", "a/1", "a/2", "a/3", "a/4", "a/5")], function(df){
...
sapply(seq_along(throuput_means[1]), function(i){
  polygon(
...
legend("bottomright", gsub("/", "", names(conditions_ma[c("a/0", "a/1", "a/2", "a/3", "a/4", "a/5")])[1]), lwd = 3, col = cols, bty = "n")
